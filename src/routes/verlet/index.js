...
class Constraint {
	...
	step() {
		...
		this.a.cpos = this.a.cpos.minus(norm.times(0.5 * diff * CONSTRAINT_DAMPING));
		this.b.cpos = this.b.cpos.plus(norm.times(0.5 * diff * CONSTRAINT_DAMPING));
	}
}
...
function Rope(shape, body) {
	shape.constraints.push(new Constraint(body, shape.bodies[shape.bodies.length - 1]));
...
function RopeSquare(pos, nrope, lrope) {
	...
	for (let i = 0; i < nrope; i++) {
		Rope(s, new Body(pos.minus(new vec(0, i * lrope)), CORPSE_SIZE, [0, 0, 0]));
	}
...
function Grid(pos, points, sparse) {
	...
	for (let i = 0; i < points; i++) {
		for (let j = 0; j < points; j++) {
			s.bodies.push(new Body(pos.plus(new vec(j * sparse, i * sparse)), CORPSE_SIZE, [0, 0, 0]));
		}
	}
